...
rsync -avz --exclude "*node_module*" --exclude ".*" --exclude "Code" "$ROOT_DIR" "$BACKUP_DIR"

# backup important hidden directories and files
rsync -avz "$ROOT_DIR/.config" "$ROOT_DIR/.ssh" "$ROOT_DIR/.gitconfig" "$ROOT_DIR/.local" "$BACKUP_DIR"
